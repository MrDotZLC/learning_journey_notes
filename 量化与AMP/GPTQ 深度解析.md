...
3. **分 Block 循环**（共 $K$ 个 Block）：
	   - **Block 内**：
		   ...
		   - 计算误差：$\mathbf{err} = (W_{:,i} - \hat{W}_{:,i}) \;/\; [\mathbf{L}^{-1}]_{ii}$。
		   - 误差补偿：$W_{:,\,j} \leftarrow W_{:,\,j} - \mathbf{err} \cdot (\mathbf{L}^{-1})_{i,j}$，对所有后续列 $j > i$ 执行。
	   - **Block 后**：
	      - $E = W[:, i:i+B] − Q[:, i:i+B]$
	      - 将 Block 累计误差矩阵 $E$ 与 $\mathbf{H}'^{-1}$ 的对应子矩阵做矩阵乘$W[:, i+B:] −= E \cdot L⁻¹[i+B:, i:i+B]^T$，批量更新后续所有列。
