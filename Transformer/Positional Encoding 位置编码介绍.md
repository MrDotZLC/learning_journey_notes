...
### 2. 核心思想
transformer中维度之间无关联，只和前后同维度有关联，同时，为表达token的整体和局部信息，使用多种频率的 **sin/cos** 函数，将位置 `pos` 转换为一个 **唯一的高维向量**，加到 token embedding。
### 3. 公式
对于位置 pos、维度 i：
...
- 特别长序列容易出现周期折返
- 含绝对位置项，导致困惑度上升，外推失败
- 同一对 token 出现在不同绝对位置时，注意力分数不同
- 模型没有专注于语义，还“记忆”绝对位置的影响
