TensorRT 的 **INT8 Calibration（8-bit 量化校准）**本质是在**离线阶段估计各 Tensor 的动态区间（dynamic range）**，从而确定 scale，在**截断误差（clipping error）**与**量化误差（rounding error）**之间取得最优平衡，使 FP32/FP16 网络安全映射到 INT8 推理。
# 一、为什么需要 Calibration？
 INT8 量化形式：
...
## 🎯 Calibration 的数学目标
 [  
T^* = \arg\min_T \left(E[e_{clip}^2] + E[e_{quant}^2]\right)  
]
 👉 最小化总量化误差
 ---
...
E[e_{clip}^2] + E[e_{quant}^2]  
 ...
 ---
 如果你希望继续深入，我可以推导：
 ✅ Clipping error 与 Rounding error 的期望表达式  
✅ 为什么 KL 与 MSE 得到不同 threshold  
✅ 激活长尾的统计学解释  
✅ Transformer 校准难点
 你是偏向理论严谨推导，还是部署经验优化？