TensorRT 是 **NVIDIA 推出的高性能深度学习推理（Inference）优化与部署框架**，核心目标是在 **NVIDIA GPU / DLA / Jetson 平台**上，以尽可能低的延迟和高吞吐量运行训练好的神经网络模型。下面从**定位、原理、核心组件、优化技术、工作流程、适用场景与局限**等方面系统介绍。
...
* 对比 **TensorRT vs ONNX Runtime / OpenVINO**
* 讲 **TensorRT + Transformer（Attention 优化）**
* 给你一个 **完整的 ONNX → TensorRT 示例**
* 或从 **源码 / 底层 CUDA 角度**深挖
