# 📘 C++ 知识笔记目录（适配 Obsidian）

## 一、C++ 基础概念
- [C++ 三大特性](#c三大特性)
- [面向对象 vs 面向过程](#面向对象和面向过程语言的区别)
- [struct 与 union 区别](#结构体struct和共同体union的区别)
- [内存对齐规则](#struct的内存对齐规则)
- [C++ 内存布局 / 程序分段](#c内存布局程序分段)
- [四种变量存储类型](#c中四种变量存储类型总结)

## 二、数组与指针
- [arr、&arr[0]、&arr 区别](#arr和arr0和arr的不同)
- [char a / char *a / char a 区别](#char-achar-achar-achar-之间的区别)
- [一维数组名与二维数组名区别](#一维数组名和二维数组名的区别)
- [数组指针与指针数组](#数组指针和指针数组)
- [const 与指针](#const与指针)
- [指针与引用的区别](#指针和引用的区别)

## 三、关键字与修饰符
- [static 与 const 用法](#static-和const分别怎么用类里面static和const可以同时修饰成员函数吗)
- [static 初始化时机与线程安全](#static初始化时机和线程安全问题)
- [局部静态变量](#c中局部静态变量的问题)
- [typedef 与 #define 区别](#typedef与define的区别)
- [#include<> 与 #include"" 区别](#include和include-的区别)
- [explicit 关键字](#explicit-关键字)
- [friend 友元类与函数](#friend友元类和友元函数)
- [inline 内联函数与宏定义](#inline-内联函数与宏定义)
- [struct 与 typedef struct 区别](#-struct-和-typedef-struct)
...
- [深拷贝与浅拷贝](#深拷贝和浅拷贝)
- [虚函数机制与多态实现](#c虚函数)
- [this 指针详解](#c中this指针相关问题)
- [C++ 内联优化等级 -O0/O1/O2/O3](#o0o1o2o3优化)

## 五、常用关键机制
- [const 与 #define 常量区别](#用const和define定义常量哪个更好)
- [typedef 与 #define 区别](#typedef与define的区别)
- [extern 与 static 用法总结](#c中四种变量存储类型总结)
- [register 寄存器变量](#c中四种变量存储类型总结)
- [this 指针生命周期与作用](#c中this指针相关问题)
- [构造函数与析构函数行为](#c虚函数)
...
常量指针表明初始化后的指针指向的地址是不能改变的，但这块地址上的存储的值可以改变，地址跟随一生。所以`p2= &a是错误的，而*p2 = a 是正确的。`
...
| 含义     | 值不可修改     | 编译期常量           |
| 生效时机   | 运行期或编译期   | 编译期             |
| 初始化要求  | 可运行时赋值    | 必须编译期可计算        |
| 可作模板参数 | ❌ 否       | ✅ 是             |
| 常见用途   | 只读变量、成员函数 | 常量表达式、数组长度、模板参数 |
```
const int a = 5;          // ✅ 运行期或编译期可初始化
constexpr int b = 5;      // ✅ 编译期常量

int x = 3;
const int c = x;          // ✅ 可运行时赋值
constexpr int d = x;      // ❌ 编译错误（x 非常量表达式）
...
5. 在C++中，指针和引用经常用于函数的参数传递，然而，指针传递参数和引用传递参数是有本质上的不同的：指针传递参数本质上是值传递的方式，它所传递的是一个地址值。值传递的特点是被调函数对形式参数的任何操作都是作为局部变量进行，不会影响主调函数的实参变量的值。而在引用传递过程中， 被调函数的形参虽然也作为局部变量在栈中开辟了内存空间，但是这时存放的是由主调函数放进来的实参变量的地址（指针放的是实参变量地址的副本）。
...
左值是可以取地址、位于赋值符号左边的值，就记住，左值是表达式结束（不一定是赋值表达式）后依然存在的对象。
...
对于代码1。test3相当于标识符，而test4相当于变量类型.定义一个结构体变量为`test4 t`，即`struct test3 = test4`
...
首先不是BUG，这种情况属于未定义的行为，而未定义的行为不会导致编程失败。
...
auto使用的是模板实参推断（Template Argument Deduction）的机制。
...
C++中的异常处理机制主要使用try、throw和catch三个关键字。就是程序中需要throw一个类型的异常，然后catch住这个类型的异常进行处理。
