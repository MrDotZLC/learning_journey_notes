...
| **内存类型**                      | **访问延迟（时钟周期）** | **说明**                                                                            |
| ----------------------------- | -------------- | --------------------------------------------------------------------------------- |
| **寄存器（Register）**             | **1 个周期**      | 位于 **SM** 内部，存储每个线程的局部数据。每个线程拥有自己的寄存器，速度非常快。寄存器不足时，数据会溢出到 **局部内存**。               |
| **共享内存（Shared Memory）**       | **≈ 10 个周期**   | 位于 **SM** 内部，同一线程块内的线程共享，存储线程块之间共享的数据。访问速度较快，但具体延迟会根据架构有所差异。                      |
| **L1 缓存（L1 Cache）**           | **几十个周期**      | 位于 **SM** 内部，是全局内存与执行单元之间的缓存层，提供比全局内存更快的访问速度。                                     |
| **L2 缓存（L2 Cache）**           | **数百个周期**      | 位于整个 GPU 中，所有 **SM** 共享这一缓存。用于减少对全局内存的访问，并缓存来自各个 **SM** 的数据请求。访问速度相对较慢，但比全局内存快得多。 |
| **常量内存（Constant Memory）**     | **几周期**        | 只读，位于全局内存，适用于存储不变的数据。常量内存访问利用 **广播机制**，当多个线程访问相同数据时，能够非常快速地读取。                    |
| **局部内存（Local Memory）**        | **几百个周期**      | 位于 **全局内存**，用于存储寄存器溢出的局部数据。访问速度慢，类似于全局内存的访问延迟。                                    |
| **纹理内存（Texture Memory）**      | **数百个周期**      | 专门为图形渲染优化的内存类型，存储图像数据。纹理内存适合二维数据存取，提供硬件插值，支持缓存机制，因此在图像处理等任务中比全局内存快。               |
| **表面内存（Surface Memory）**      | **数百个周期**      | 表面内存是纹理内存的扩展，支持 **只读或读写** 操作，适合存储图像数据和多维数组。                                       |
